...
const UpdateElectronics = () => {
  ...
  useEffect(() => {
    axios.get(`https://e-commerce-project-dashboard.onrender.com/categories/electronics/${id}`)
      .then(result => {
  ...
  const Update = async (e) => {
    ...
    axios.put(`https://e-commerce-project-dashboard.onrender.com/categories/electronics/${id}`, {
      name,
      price: Number(price),
      ...
      .then(() => {
        navigate(`/addremoveelectronicsitems?page=${currentPage}`);
      })
      .catch(err => console.log(err));
  };

  return (
    <div className="flex justify-center items-center bg-gradient-to-r from-emerald-100 to-emerald-200 px-4 pt-8 pb-8">
      <div className="bg-white/90 backdrop-blur rounded-xl px-8 py-3 w-full max-w-xl shadow-lg border border-gray-200">
        <form className="w-full space-y-6" onSubmit={Update}>
          ...
          <button
            type="submit"
            className="w-full bg-emerald-200 hover:bg-emerald-300 text-white py-3 rounded-lg flex items-center justify-center gap-2 transition duration-200"
          >
            <FaEdit />
...
export default UpdateElectronics;
