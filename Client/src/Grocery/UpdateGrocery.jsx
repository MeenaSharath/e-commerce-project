...
const UpdateGrocery = () => {
  ...
  useEffect(() => {
    axios.get(`https://e-commerce-project-dashboard.onrender.com/categories/grocery/${id}`)
      .then(result => {
  ...
  const Update = async (e) => {
    ...
    axios.put(`https://e-commerce-project-dashboard.onrender.com/categories/grocery/${id}`, {
      name,
      price: Number(price),
      ...
      .then(() => {
        navigate(`/addremovegroceryitems?page=${currentPage}`);
      })
      .catch(err => console.log(err));
  };

  return (
    <div className="flex justify-center items-center bg-gradient-to-r from-indigo-100 to-indigo-200 px-4 pt-8 pb-8">
      <div className="bg-white/90 backdrop-blur rounded-xl px-8 py-3 w-full max-w-xl shadow-lg border border-gray-200">
        <form className="w-full space-y-6" onSubmit={Update}>
          ...
          <button
            type="submit"
            className="w-full bg-indigo-200 hover:bg-indigo-300 text-white py-3 rounded-lg flex items-center justify-center gap-2 transition duration-200"
          >
            <FaEdit />
