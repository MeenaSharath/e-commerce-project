...
const AddGrocery = () => {
  ...
  const Submit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3001/createGrocery", {
        name,
        price: Number(price),
      ...
      navigate('/addremovegroceryitems');
    } catch (err) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-lime-100 to-lime-200 px-4 pb-10">
      <div className="backdrop-blur-lg bg-white/80 border border-gray-300 rounded-2xl px-8 py-3 w-full max-w-xl shadow-xl">
        <form className="w-full" onSubmit={Submit}>
          ...
          <button
            type="submit"
            className="w-full bg-lime-200 text-white py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-lime-300 transition"
          >
