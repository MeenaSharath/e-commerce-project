...
const UpdateEntertainment = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3001/getEntertainment/${id}`)
      .then(result => {
  ...
  const Update = async (e) => {
    ...
    axios.put(`http://localhost:3001/updateEntertainment/${id}`, {
      name,
      price: Number(price),
      ...
      .then(() => {
        navigate(`/addremoveentertainmentitems?page=${currentPage}`);
      })
      .catch(err => console.log(err));
  };
    ...
    <div className="flex justify-center items-center bg-gradient-to-r from-fuchsia-100 to-fuchsia-200 px-4 pt-8 pb-8">
      <div className="bg-white/90 backdrop-blur rounded-xl px-8 py-3 w-full max-w-xl shadow-lg border border-gray-200">
        <form className="w-full space-y-6" onSubmit={Update}>
          ...
          <button
            type="submit"
            className="w-full bg-fuchsia-200 hover:bg-fuchsia
            -300 text-white py-3 rounded-lg flex items-center justify-center gap-2 transition duration-200"
          >
            <FaEdit />
...
export default UpdateEntertainment;
